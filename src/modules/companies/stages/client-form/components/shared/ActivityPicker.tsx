...
/**
 * Selector jerárquico de actividad económica en un solo dropdown.
 *
 * Internamente navega tres niveles: Sector → Categoría → Actividad,
 * con breadcrumb para volver atrás. Permite búsqueda libre en el nivel
 * de actividad (que es donde hay más cardinalidad).
 *
 * Sólo `activity.id` se persiste en el form.
 */
export function ActivityPicker({
	...
	onChange,
	disabled,
	placeholder = 'Selecciona tu actividad económica',
}: Props) {
	...
	// Pre-cargar sector/category cuando ya hay actividad seleccionada (draft)
	const selected = useMemo(
		() => activities.find((a) => String(a.id) === String(value)),
		[activities, value],
	);
	useEffect(() => {
		if (!selected) return;
