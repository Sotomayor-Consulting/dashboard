import type { Company, CompanyTableRow } from '@modules/companies/types';
...
export type StatusByStage = { [K in Stage]: (typeof STAGE_STATUS_MAP)[K][number] };
...
type StageStatusPair = {
	[K in Stage]: { current_stage: K; current_status: StatusByStage[K] };
}[Stage];

export type Incorporation = IncorporationBase & StageStatusPair;
export type IncorporationWithRawCompany = Incorporation & { company?: Company | null };
...
export const getStatusesForStage = (stage: Stage) => STAGE_STATUS_MAP[stage];
