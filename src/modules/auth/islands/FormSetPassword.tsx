...
interface FormSetPasswordProps {
	...
	status?: string | null;
	message?: string | null;
	/** false cuando el usuario llegó sin sesión (enlace expirado o ya usado) */
	hasSession?: boolean;
}
	...
	const handleSubmit = async (event: SyntheticEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (!response.ok || payload.ok === false) {
				setClientError(payload.error ?? 'No se pudo crear la contraseña.');
				return;
			}
					...
					{!hasSession ? (
						<>
							<div className="space-y-2 text-center">
								...
								<p className="text-sm text-slate-500 dark:text-slate-400">
									Tu enlace de invitación expiró o ya fue utilizado. Por
									seguridad los enlaces son de un solo uso, pero puedes pedir
									uno nuevo tú mismo.
								</p>
							</div>
							...
							<div className="w-full space-y-3">
								<a
									...
									className={cn(
										...
										'h-11 w-full rounded-xl no-underline',
									)}
								>
									Enviar un nuevo enlace a mi correo
								</a>
								<a
									href="/sign-in"
									className="block text-center text-sm text-slate-500 no-underline hover:underline dark:text-slate-400"
								>
							...
							<p className="px-4 text-center text-xs text-slate-500 dark:text-slate-400">
								Escribe tu correo en el siguiente paso y te llegará un enlace
								nuevo para crear tu contraseña. Si no recibes el correo,
						...
						<>
							<div className="space-y-2 text-center">
								...
								<p className="text-sm text-slate-500 dark:text-slate-400">
									Tu cuenta fue creada por invitación. Crea tu contraseña para
									activar el acceso.
								</p>
							</div>
							...
							>
								...
								<div className="flex items-start gap-3">
									<input
										...
										required
										disabled={pending}
										className="mt-0.5 size-4 shrink-0 rounded-xs border border-slate-300 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#8c681d]/30 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-600"
									/>
									<label
										htmlFor="terms"
							...
							<p className="px-8 text-center text-xs text-slate-500 dark:text-slate-400">
								...
								y{' '}
								<a
									href="https://sotomayorconsulting.com/inicio/privacidad/"
									target="_blank"
									rel="noopener"
									className="font-medium text-slate-900 hover:underline dark:text-white"
								>
