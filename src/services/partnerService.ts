...
export async function getReferralsByEmail(email: string): Promise<ServiceResponse<OdooPartner[]>> {
  try {
    // Intentamos conectar con Odoo...
    const parents = await executeKw('res.partner', 'search_read', 
      [[['email', '=', email]]], 
      { fields: ['id'], limit: 1 }
    ) as OdooPartner[];
    ...
    const referrals = await executeKw('res.partner', 'search_read',
      [[['x_referido_id', '=', parents[0].id]]],
      { fields: ['name', 'email', 'phone', 'city', 'sale_order_ids','create_date'], limit: 100 }
    ) as OdooPartner[];
