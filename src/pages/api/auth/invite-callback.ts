	...
	try {
		if (authCode) {
			...
			if (!access_token || !refresh_token) {
				return new Response('No se recibieron tokens en la sesión', {
					status: 500,
				});
			}
		...
		if (inviteToken) {
			...
			if (!session)
				return new Response(
					'No se pudo obtener sesión desde el token de invitación',
					{ status: 500 },
				);
		...
		return new Response("Parámetros inválidos: se requiere 'code' o 'token'", {
			status: 400,
		});
	} catch (err: any) {
