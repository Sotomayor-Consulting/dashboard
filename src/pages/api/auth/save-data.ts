...
export const POST: APIRoute = async ({ request, cookies, redirect, url }) => {
  try {
    ...
    if (!nombre_1) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("El nombre 1 es obligatorio")}`);
    }
     if (!nombre_2) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("El nombre 2 es obligatorio")}`);
    }
     if (!nombre_3) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("El nombre 3 es obligatorio")}`);
    }
