...
export const POST: APIRoute = async ({ request, cookies, redirect, url }) => {
  ...
  try {
    ...
    if (Object.keys(payload).length === 1) { // Solo tiene updated_at
    ...
    if (error) {
      const msg = encodeURIComponent(`Error al actualizar servicio: ${error.message}`);
      return redirect(`${back}?status=error&msg=${msg}`);
    }

    return redirect(`${back}?status=success&msg=${encodeURIComponent("Servicio actualizado correctamente")}`);
