...
export const POST: APIRoute = async ({ request, cookies, redirect, url }) => {
  ...
  try {
    ...
    if (!at || !rt) {
      return redirect(`${backPath()}?status=error&msg=${encodeURIComponent("No autenticado")}`);
    }
    ...
    if (sErr) {
      return redirect(`${backPath()}?status=error&msg=${encodeURIComponent("No fue posible establecer su sesión")}`);
    }
    ...
    const { data: isAdminRes, error: rpcErr } = await supabase.rpc("is_admin_global_empresas");
    const isAdmin = !rpcErr && Boolean(isAdminRes);
    if (!isAdmin) {
      return redirect(`${backPath()}?status=error&msg=${encodeURIComponent("No autorizado")}`);
    }
    ...
    const formId =
      ...
      "";
    if (!formId) {
      return redirect(`${backPath()}?status=error&msg=${encodeURIComponent("Falta el ID del formulario")}`);
    }
    ...
    if (schemaJsonRaw) {
      try {
        ...
        return redirect(`${backPath()}?status=error&msg=${encodeURIComponent("JSON del formulario inválido")}`);
      }
    }
    ...
    if (temaRaw && temaRaw.length > 0) {
      try {
        ...
        return redirect(`${backPath()}?status=error&msg=${encodeURIComponent("JSON del tema inválido")}`);
      }
    }
    ...
    if (Object.keys(payload).length === 1) {
      return redirect(`${backPath()}?status=success&msg=${encodeURIComponent("Sin cambios")}`);
    }
    ...
    if (updErr) {
      ...
      return redirect(`${backPath()}?status=error&msg=${encodeURIComponent(msg)}`);
    }

    return redirect(`${backPath()}?status=success&msg=${encodeURIComponent("Formulario actualizado correctamente")}`);
  } catch (e: any) {
