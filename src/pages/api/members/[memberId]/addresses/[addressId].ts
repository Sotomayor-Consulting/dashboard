...
export const PATCH: APIRoute = async ({ request, cookies, params }) => {
	...
	try {
		...
		console.error('[member_addresses:update]', error);
...
export const DELETE: APIRoute = async ({ request, cookies, params }) => {
	...
	try {
		const address = await softDeleteMemberAddress(
			context.supabase,
			memberId,
			addressId,
			context.user.id,
		...
		console.error('[member_addresses:delete]', error);
		const message = error instanceof Error ? error.message : 'INTERNAL_ERROR';
