...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const template = await readTemplate(
			defaultTpl,
			templateFile as File | undefined,
		);

		const rawData = dataText
			? JSON.parse(dataText)
			: {
					id: 4242,
		...
		const total = (rawData.products ?? []).reduce(
			(a: number, it: any) => a + Number(it.priceTotal ?? 0),
			0,
		);
		const prepared = {
			...rawData,
...
export const GET: APIRoute = async (ctx) => {
	...
	return POST({
		...ctx,
		request: new Request(ctx.url, { method: 'POST', body: form }),
	});
};
