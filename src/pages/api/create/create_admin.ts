...
export const POST: APIRoute = async ({ request, cookies, redirect, url }) => {
  ...
  const { data: isAdminRes, error: rpcErr } = await supabase.rpc("is_admin", { uid: user.id });
  ...
  if (!userId) {
    return redirect(`${back}?status=error&msg=${encodeURIComponent("Falta user_id (UUID de auth.users)")}`);
  }
  ...
  const rol_id = rolIdRaw && !Number.isNaN(Number(rolIdRaw)) ? Number(rolIdRaw) : undefined;
  ...
  return redirect(`${back}?status=success&msg=${encodeURIComponent("Usuario creado")}`);
};
