...
export const POST: APIRoute = async ({ request, cookies, redirect, url }) => {
  ...
  try {
    ...
    if (!nombre) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("Falta el nombre del rol")}`);
    }
    ...
    return redirect(`${back}?status=success&msg=${encodeURIComponent("Rol creado correctamente")}`);
