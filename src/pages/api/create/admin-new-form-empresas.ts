...
export const POST: APIRoute = async ({ request, cookies, redirect, url }) => {
  ...
  try {
    ...
    if (sErr) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("No fue posible establecer la sesión.")}`);
    }
    ...
    const empresaSlugFromQS = url.searchParams.get("empresa_slug") || url.searchParams.get("slug");
    ...
    if (!empresaId) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("No se pudo determinar la empresa del formulario.")}`);
    }

    // 4) Validaciones mínimas
    if (!titulo) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("Falta el título.")}`);
    }
    if (!slugRaw) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("Falta la categoría (slug).")}`);
    }
    if (!schemaJsonRaw) {
      return redirect(`${back}?status=error&msg=${encodeURIComponent("Falta el JSON del formulario.")}`);
    }
    ...
    const nextRevision =
      revRows && revRows.length > 0 && Number.isInteger(revRows[0].revision as number)
