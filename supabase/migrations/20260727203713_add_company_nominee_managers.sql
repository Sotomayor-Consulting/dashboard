...
-- Roster de staff habilitado para figurar como Nominee Manager. Desacopla
-- la identidad "legal" que se imprime en el filing estatal de la cuenta de
-- usuario interna (usuarios), igual que registered_agents desacopla el
-- agent_name/dirección del provider.
...
-- A diferencia de registered_agents (SELECT true para cualquier
-- authenticated), este roster es operativo/interno: solo staff lo lee.
...
-- Vigencias de asignación por empresa. end_date IS NULL = asignación activa;
-- el historial se conserva cerrando la fila anterior en vez de update in-place
-- (mismo patrón que company_registered_agents).
...
-- Un solo nominee manager vigente por empresa a la vez.
